...
3. 无同步方案：①本身不涉及共享数据，无须任何同步措施。②可重入代码定义：（简单判断原则）一个方法，返回结果可预测，只要输入了相同数据，返回相同结果，则满足可重入性要求，并非所有线程安全代码都是可重入的。③线程本地存储，若变量要多线程访问，可用Volatile声明，若线程独占则使用Thread、ThreadMap键值对。 
